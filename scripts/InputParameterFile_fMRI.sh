...
# Note that resting-state data is processed the same way as task data, so assign resting-state data to a "task" in the Tasklist
   ...
   DistortionCorrection="TOPUP" #FIELDMAP or TOPUP, distortion correction is required for accurate processing
       ...
       ### If you use DistortionCorrection="TOPUP" change variables in this part and read additional notes at the bottom of this file, otherwise skip it
        SpinEchoPhaseEncodeNegative="%TaskName%_SEtopup_RL.nii.gz" #For the spin echo field map volume with a negative phase encoding direction (LR in HCP data)
        SpinEchoPhaseEncodePositive="%TaskName%_SEtopup_LR.nii.gz" #For the spin echo field map volume with a positive phase encoding direction (RL in HCP data)
        TopUpConfigForFUNC="${HCPPIPEDIR_Config}/b02b0.cnf" #Topup config if using TOPUP

        UnwarpdirForFUNC="-x"  
        DwellTime="0.00058" # Effective Echo Spacing (or Dwelltime) of fMRI image in seconds - note that this is the echo spacing divided by any in-plane acceleration factor
