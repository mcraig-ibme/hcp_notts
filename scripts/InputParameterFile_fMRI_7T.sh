...
#Naming conventions:	StudyFolder/SubjectFromSubjlist/image.nii.gz 
  ...
  # T1w
  T1wInputImages="T1A.nii.gz"

  # T2w (if it exists, otherwise set T2wInputImage to "NONE") 
  T2wInputImages="T2A.nii.gz"  # filename of T2w image or "NONE" if not used 
...
# Note that resting-state data is processed the same way as task data, so assign resting-state data to a "task" in the Tasklist
    ...
    fMRITimeSeries="%TaskName%.nii.gz"      # Make sure the data have an even number of slices!!!
   ...
   DistortionCorrection="TOPUP" #FIELDMAP or TOPUP, distortion correction is required for accurate processing
       ...
       ### If you use DistortionCorrection="TOPUP" change variables in this part and read additional notes at the bottom of this file, otherwise skip it
        SpinEchoPhaseEncodeNegative="%TaskName%_SEtopup_AP.nii.gz" #For the spin echo field map volume with a negative phase encoding direction (LR in HCP data)
        SpinEchoPhaseEncodePositive="%TaskName%_SEtopup_PA.nii.gz" #For the spin echo field map volume with a positive phase encoding direction (RL in HCP data)
        TopUpConfigForFUNC="${HCPPIPEDIR_Config}/b02b0.cnf" #Topup config if using TOPUP

        UnwarpdirForFUNC="-y"  
        DwellTime="0.00016" # Effective Echo Spacing (or Dwelltime) of fMRI image in seconds - note that this is the echo spacing divided by any in-plane acceleration factor
...
### Gradient non-linearity distortion correction (very important for HCP data!) ###
  ...
  GradientDistortionCoeffs="/home/fs0/stam/scratch/Pipelines/global/config/7TAS_coeff_SC72CD.grad"
  GradientDistortionCoeffsForFUNC="/home/fs0/stam/scratch/Pipelines/global/config/7TAS_coeff_SC72CD.grad"
